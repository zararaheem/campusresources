import { FIELD_DEFS, RESOURCE_LINKS, DEFAULT_FIELD_VALUES } from './seed';

const PLACEHOLDER_RE = /\{\{\s*([a-z0-9_]+)\s*\}\}/gi;

// Sections that families sign through the in-app popup. Signing is captured by
// the "Review & sign" form (name + typed signature), so the printed copy no
// longer carries "Signature: ____" lines. A section is signable if it's one of
// these known contracts, or (for custom campus sections) still includes a
// literal "Signature:" line.
const SIGNABLE_KEYS = new Set(['two-hour-learning', 'student-contract']);
function isSignable(key, rawBody) {
  return SIGNABLE_KEYS.has(key) || /Signature:/i.test(rawBody || '');
}
...
export function resolveHandbook(sections, location) {
  const fields = { ...DEFAULT_FIELD_VALUES, ...(location?.fields || {}) };
  ...
  const resolved = ordered
    .map((s) => {
      ...
      return {
        ...
        signable: isSignable(s.key, rawBody),
      };
    })
    .filter(Boolean);
  ...
  for (const ex of extras) {
    ...
    resolved.push({
      ...
      overridden: false,
      extra: true,
      signable: isSignable(ex.key, ex.body),
    });
  }
