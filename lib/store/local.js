...
function seedData() {
  ...
  return { sections, locations, editors, signatures: [] };
}
  ...
  return {
    ...
    async addSignature(rec) {
      return mutate((d) => {
        ...
        const row = { id: crypto.randomUUID(), signed_at: new Date().toISOString(), ...rec };
        d.signatures.push(row);
        return row;
      });
    },
    async listSignatures({ code } = {}) {
      const all = (load().signatures || []).slice();
      const rows = code ? all.filter((s) => s.location_code === code) : all;
      return rows.sort((a, b) => (b.signed_at || '').localeCompare(a.signed_at || ''));
    },
  };
}
